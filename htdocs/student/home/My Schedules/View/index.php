	...
	<main>
		<div class="index-banner">
			...
			<table>
				...
				$id = $_SESSION['u_id'];
				$sql = "SELECT * FROM student_schedules WHERE student_id = '$id';";
						$result = mysqli_query($conn, $sql);
						$resultcheck = mysqli_num_rows($result);

						if($resultcheck > 0) {
							while ($row = mysqli_fetch_assoc($result)) { 
								echo "<tr><td>".$row['student_schedule_name']."</td>";
								echo "<td>".$row['student_schedule_type']."</td>";
								echo "<td>".$row['student_schedule_start']."</td>";
								echo "<td>".$row['student_schedule_end']."</td>";
								echo "<td>".$row['student_schedule_date']."</td>";
								echo "<td>".$row['student_schedule_note']."</td></tr>";
							}
						}
				echo '</table></article>
		</div>
