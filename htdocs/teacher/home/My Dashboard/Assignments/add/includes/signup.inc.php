...
if (isset($_POST['submit'])) { //to check if the button have been clicked
	...
	if (empty($name) || empty($type) || empty($weight) || empty($grade1) || empty($grade2) || empty($date)) { //to check if any have been left empty
		header("location: ../index.php?signup=empty");
					...
					$sql = "insert into assignments (assignment_name, assignment_type, assignment_subject, assignment_weight, assignment_grade, assignment_grade2, assignment_date) value ('$name', '$type', '$subject', '$weight', '$grade1', '$grade2', '$date')";  
